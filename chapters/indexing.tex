...
In the event of a lookup failure, the environment has at least the bindings of its initial state.

\newcommand{\D}{\mathcal{D}}
\newcommand{\E}{\mathcal{E}}

\begin{theorem}[Purity]
Indexing an \LF kind, type or term using the Hoare-style formalism restores the referencing environment to its initial state when it succeeds.
\begin{enumerate}
...
\par
Then, inductively on $n$, $\Xi_n = \Xi_{n - 1} = \dots = \Xi_1 = \Xi_0 = \Xi$.
\end{itemize}
\item
...
\par
By the induction hypothesis on $\D_2$, $\deduce{\Shift{\Psi}{\Xi''} \vdash \IndexesType{B}{\tilde B}}{\E_2}$.
\par
Then,
\begin{equation*}
\infer{
...
By the induction hypothesis on $\D_1$, $\deduce{\Xi \vdash \IndexesType{A}{\tilde A}}{\E_1}$ and $\Xi'' = \Xi$.
\par
By the induction hypothesis on $\D_2$, $\deduce{\Push{\Psi}{x : \LFTerm}{\Xi''} \vdash \IndexesType{B}{\tilde B}}{\E_2}$.
\par
Then,
\begin{equation*}
\infer{
...
\par
Inductively on $k$, $\Xi_k = \Xi$ for each $k \in \set{1, 2, \dots, n}$.
\par
Then,
\begin{equation*}
\infer{
...
\par
By the induction hypothesis on $\D'$, $\deduce{\Push{\Psi}{x : \LFTerm}{\Xi} \vdash \IndexesTerm{M}{\tilde M}}{\E'}$ and $\Xi'' = \Xi$.
\par
Then,
\begin{equation*}
\infer{
...
\par
Inductively on $n$, $\Xi_n = \Xi_{n - 1} = \cdots = \Xi_0 = \Xi$.
\par
Then,
\begin{equation*}
\infer{

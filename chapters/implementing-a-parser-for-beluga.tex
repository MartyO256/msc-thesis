...
Specifically, since the index level \LF of \Beluga is strongly-normalizing, and since reconstruction for \LF terms and types operates best with objects in canonical form, then it is sensible to require the user to only write \LF terms and types in canonical form.
Additionally, since \Beluga's type-checking algorithm is bidirectional, it also appeared sensible to split computation-level expressions into two syntactic classes, namely type-synthesizing expressions and type-checkable expressions.
Implementation efforts revealed that these syntactic constraints are too stringent to be imposed at the level of the parser.
...
% TODO: Explain the solution, refer to the appendix
...
\end{grammar}
\caption[The grammar for parsing contextual \LF terms in normal form in the legacy parser.]{%
...
The productions \synt{clf-term-application}, \synt{clf-head} and \synt{clf-type-atomic} showcase a dramatic increase in complexity for parsing contextual \LF terms and types.
This is a consequence of trying to reject all non-normal forms at the level of the parser.
...
Since the grammar for \LF objects was relaxed to support non-canonical forms, the parser recognizes the invalid substitution, and then disambiguation identifies that it should be rejected.
}
\end{subfigure}

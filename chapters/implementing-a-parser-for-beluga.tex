...
Additionally, since \Beluga's type-checking algorithm is bidirectional, it also appeared reasonable to split computation-level expressions into two syntactic classes, namely type-synthesizing expressions and type-checkable expressions.
Implementation efforts revealed however that these syntactic constraints are too stringent to be imposed at the level of the parser.
...
We merge syntactic categories having common syntaxes that appear in ambiguous positions into one production.
We call this blurring the grammar.
...
A similar technique was used in \Twelf since its shift-reduce parser could not perform disambiguation during parsing.

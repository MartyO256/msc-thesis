...
Context-sensitive languages necessitate additional data about the context in which parts of the program appears in order to parse it.
...
\Beluga became context-sensitive when modifications were made to its syntax, specifically to improve the syntac of contextual objects and to add user-defined operators as in \Twelf for \LF type-level and term-level constants.
...
As such, the recursive descent parselet for $\rightarrow$ and $\leftarrow$ operators only allow one or the other operator to appear.
...
The performance impact of this lookahead has not been measured, but it appears to be negligeable based on the performance of running the entire processing pipeline on the numerous examples that are part of the system test suite.
...
Since we need a parselet per precedence level, and most static operators in \Beluga have distinct precedences, the resulting increase in the recursion depth for recursive descent parsing could however deteriorate the runtime performance of the parser.
Arguably, this design would be easier to extend, and could provide more opportunities for improving error messages in the disambiguation phase.

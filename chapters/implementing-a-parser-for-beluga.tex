...
As the \Beluga language grew, so did the complexity of its grammar, such that the errors generated by \CamlpFour were deemed not sufficiently informative to the end-user.
...
The solution that was ultimately put in place was to introduce two additional token kinds \synt{dot-identifier} and \synt{dot-integer} in the lexer for identifiers and numbers immediately prefixed by a dot.

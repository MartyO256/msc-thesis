...
For example, it is impossible in \Beluga~\texttt{v1.0} for -level coinductive type constant to shadow an inductive one simply because the declaration table for the former kind is always looked up after the declaration table for the latter kind.
...
The stacks of bindings provide a means of recalling what earlier definitions were assigned to identifiers as the traversal exits a scope.
...
Binding tree & $ \BindingsTree $ & $ \Coloneqq $ & $ \{x_i = \mathcal{S}_i\}_{i \in \set{1, 2, \dots, n}} $\\
...
The set of toplevel identifiers for frames and binding trees is defined in figure~\ref{figure:definition-domain-frames}.
...
{\footnotesize
\begin{tabular}{lcll}
$\Domain(\Frame)$ & $=$ & $\Domain(\Bindings)$ & $\Frame = \Bindings$\\
$\Domain(\Module)$ & $=$ & $\Domain(\Bindings)$ & $\Module = (\Bindings, \_)$\\
$\Domain(\Pattern)$ & $=$ & $\Domain(\Bindings_{\text{p}})$ & $\Pattern = (\Bindings_{\text{p}}, \_)$\\
$\Domain(\Bindings)$ & $=$ & $\Domain(\BindingsTree)$ & $\Bindings=(\BindingsTree, \_, \_, \_)$\\
$\Domain(\BindingsTree)$ & $=$ & $\set{x_1, x_2, \dots, x_n}$ & $\BindingsTree = \set{x_i = \EntryStack_i}_{i \in \set{1, 2, \dots, n}}$
\end{tabular}
}
...
Inner pattern-bound variables are introduced by binders in the pattern, like $c$ and $x$ in the contextual lambda pattern $[c : \mathsf{tm} \rightarrow \mathsf{tm} \vdash \mathsf{lam} (\textbackslash x. c\; x)]$.
...
This means that inner pattern-bound variables are added to $\Bindings_{\text{p}}$, and free variables are added to $\Bindings_{\text{e}}$.
...
A \Beluga signature with holes \texttt{\verbhole{?h1}}, \texttt{\verbhole{?h2}} and \texttt{\verbhole{?h3}}, used to showcase how a uniform referencing environment is updated when dealing with namespaces.
}
...
This showcases in particular how indexing context sizes are carried over from outer frames, how constant declarations in modules kept track of, and how frames are constructed for pattern-matching body expressions.
}
...
The operations that add or remove bindings only affect the topmost frame in the referencing environment so as to respect the nesting structure of frames.
...
%\begin{table}
%\centering
%% TODO: FIXME: Deep pattern-matching, specific rules for patterns, fix rules to biding stacks, need to introduce different lookup functions for patterns
%\begin{tabular}{lcl|l}
%$\Lookup{x_1{.}x_2{.}\cdots{.}x_n}{\Xi}$ & $=$ & $\begin{cases}
%\Lookup{x_1{.}x_2{.}\cdots{.}x_n}{\mathcal{F}}\\
%\Lookup{x_1{.}x_2{.}\cdots{.}x_n}{\Xi'}\\
%\Fail
%\end{cases}$ &  \begin{tabular}{l}
%$\Xi = (\Xi'; \mathcal{F}), x_1 \in \operatorname{domain}(\mathcal{F})$\\
%$\Xi = (\Xi'; \mathcal{F}), x_1 \notin \operatorname{domain}(\mathcal{F})$\\
%$\Xi = \cdot$
%\end{tabular}\\
%$\Lookup{x_1{.}x_2{.}\cdots{.}x_n}{\Frame}$ & $=$ & $\Lookup{x_1{.}x_2{.}\cdots{.}x_n}{\Bindings}$ & $\Frame = \Bindings$\\
%$\Lookup{x_1{.}x_2{.}\cdots{.}x_n}{\Module}$ & $=$ & $\Lookup{x_1{.}x_2{.}\cdots{.}x_n}{\Bindings}$ & $\Module = (\Bindings, \BindingsTree)$\\
%$\Lookup{x_1{.}x_2{.}\cdots{.}x_n}{\Pattern}$ & $=$ & $\Lookup{x_1{.}x_2{.}\cdots{.}x_n}{\Bindings_{\text{e}}}$ & $\Pattern = (\Bindings_{\text{e}}, \Bindings_{\text{p}})$\\
%$\Lookup{x_1{.}x_2{.}\cdots{.}x_n}{\Bindings}$ & $=$ & $\Lookup{x_1{.}x_2{.}\cdots{.}x_n}{\BindingsTree}$ & $\Bindings = (\BindingsTree, |\Psi|, |\Delta|, |\Gamma|)$\\
%$\Lookup{x_1{.}x_2{.}\cdots{.}x_n}{\BindingsTree}$ & $=$ & $\begin{cases}
%\Lookup{x_2{.}\cdots{.}x_n}{\mathcal{S}_k}\\
%\Fail
%\end{cases}$ & \begin{tabular}{l}
%$\BindingsTree = \{x_i = \mathcal{S}_i\}_{i \in \set{1, 2, \dots, m}}$, $\exists k, x_k = x_1$\\
%$\BindingsTree = \{x_i = \mathcal{S}_i\}_{i \in \set{1, 2, \dots, m}}$, $\nexists k, x_k = x_1$
%\end{tabular}\\
%$\Lookup{x_1{.}x_2{.}\cdots{.}x_n}{\mathcal{S}}$ & $=$ & $\begin{cases}
%\Lookup{x_1{.}x_2{.}\cdots{.}x_n}{\BindingsTree}\\
%e\\
%\Fail
%\end{cases}$ & \begin{tabular}{l}
%$n > 1$, $\mathcal{S} = \mathcal{S}'; (e, \BindingsTree)$\\
%$n = 1$, $\mathcal{S} = \mathcal{S}'; (e, \BindingsTree)$\\
%$\mathcal{S} = \cdot$
%\end{tabular}
%\end{tabular}
...
Using the definition for referencing environments in \Beluga, the indexing phase is defined inductively as a set of translations functions.
These are of the form $\Xi \vdash \Indexes{e}{s}{\tilde e}$, with $e$ and $\tilde e$ being the input and output terms respectively.
However, that specification of indexing assumes that $\Xi$ is implemented as an immutable data structure.
...
This subsection focusses on indexing for \LF kinds, types and terms, and presents both styles of specifications $\Xi \vdash \Indexes{e}{s}{\tilde e}$ and $\Hoare{\Env = \Xi}{\Indexes{e}{s}{\tilde e}}{\Env = \Xi'}$, then demonstrates that they are equivalent.

First, the definitions for plain \LF kinds, types and terms are recalled below, with $a$ and $c$ standing for type-level and term-level constants respectively.
...
Then, the nameless counterparts of these \LF kinds, types and terms are defined hereafter, with $\tilde a$ and $\tilde c$ denoting the symbolic identifier corresponding to the type and term constants $a$ and $c$ respectively, and $\iota$ denoting the de Bruijn indices corresponding to variables $x$.
...
The fact that computations may mutate that state is reflected formally using a Hoare-style specification.
...
% TODO: Discuss scope graphs as an alternative to the formalism and implementation, argue this ends up being the same as association lists because of telescopes
...
% TODO: Reference structural editing for free using scope graphs, how the immutable list representation is suitable for live editing as opposed to compilation.

% TODO: Maybe mentiton how the Hoare-style specification can be strengthened by having a notion of equivalence modulo the operations defined for it. That allows arbitrary non-conflicting effects to occur, but makes the inference rules verbose.

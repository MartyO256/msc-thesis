...
\subsection{Formalising Unified Indexing}\label{section:formalising-unified-lexing}
...
Traversing of the dictionaries and stacks in order of addition like is done in the formalism of section~\ref{section:formalising-unified-lexing} to count up to the correct de Bruijn index defeats the purpose of using dictionaries to speed up lookups.
...
The de Bruijn index for a variable can then later be recovered simply by subtracting the current indexing context size with the size that was recorded at the binder for that variable, as illustrated in figure~\ref{figure:lf-indexing}.
...
This is the case in \Beluga, wherein $\Psi$, $\Delta$ and $\Gamma$, the \LF context, the meta-level context and the computation-level context respectively, are indexed separately.
...
Variables in $\Psi'$ are annotated with the size of the context before each of them were added.
}

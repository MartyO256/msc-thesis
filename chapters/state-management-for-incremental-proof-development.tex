...
The main challenge with implementing such systems is state invalidation, where an edit on part of a program triggers changes elsewhere, both in the code the user wrote and in the interpreter analysing it.
While reprocessing the affected parts of the code is a valid solution to guarantee soundness, this does not scale efficiently to large programs.
...
For instance, constraint-based type systems require mechanisms to keep track of the effects of solving contraints, so that those effects can be undone in the event that type-checking fails for a given program.
...
Additionally, subgoal may be arranged internally as a tree following the structure of a proof to allow navigating between holes.
...
In case of bugs arising during edit sessions, separate commands are available to the user to restore the state to earlier checkpoint or to entirely reload the current editor buffer.
...
These declarations are arranged in tables, with one table per kind of constant declaration.
...
A \textit{fully qualified identifier} $x_1.x_2.\cdots.x_n$ is a list of plain identifiers $x_1$, $x_2$, $\dots$, $x_n$ separated by dots denoting projection.
...
In essence, a referencing environment provides a view of a signature for name resolution, and as such is restricted to one scope at a time.
...
Since \Beluga supports defining constants in modules, and pattern-matching in computation-level programs, three distinct kinds of frames are designed to operate differently in the way bindings are added to them:
\begin{enumerate}
\item

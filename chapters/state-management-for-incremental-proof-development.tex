...
%Performance concerns aside, this design cannot support 
In its interactive mode, \Abella users are restricted to adding new declarations at the end of the session's state.
...
Public constant declarations on the other hand are brought in scope when the module is opened.
\item
...
The next subsections present two different ways of realising a referencing environment for \Beluga that supports unified indexing.
...
Some annotations are omitted for brevity.
}
...
\end{equation*}
\caption{%
Definition for the sets of identifiers in scope by scope kind.
}
\end{subfigure}
\caption[Formal definition of referencing environments for \Beluga]{%
Formal definition of referencing environments for \Beluga, modelled as association lists.
}
\end{figure}
...
\end{equation*}
\caption{%
Definition of filtering to extract the \LF context from a referencing environment.
...
\begin{equation}
\infer{\Xi \vdash x \Translates_M i \dashv \Xi}{\Lookup{x}{\Xi} = (x : \mathsf{LF}_{\mathsf{term}}) & \operatorname{index}_{\Psi(\Xi)}(x) = i}
\end{equation}

\begin{equation}
\infer{\Xi \vdash \lambda x. M \Translates_M \lambda\ M' \dashv \Xi
}{\Xi, x : \mathsf{LF}_{\mathsf{term}} \vdash M \Translates_M M' \dashv \Xi, x : \mathsf{LF}_{\mathsf{term}}}
\end{equation}
...
\begin{equation}
\infer{
	\Xi; \Pattern \vdash [x1, x2, \dots, xn \vdash M_p] \Translates_p [x1, x2, \dots, xn \vdash M_p'] \dashv \Xi; \Pattern'
}{
	\Xi; \Pattern, \private{x1}, \private{x2}, \dots, \private{xn} \vdash M_p \Translates_{Mp} M_p' \dashv \Xi; \Pattern'
}
\end{equation}

\begin{equation}
\infer{
	\Xi \vdash \mathbf{case}\ i\ \mathbf{of}\ \{p_k \Rightarrow e_k\}_k \Translates_e \mathbf{case}\ i'\ \mathbf{of}\ \{p_k' \Rightarrow e_k'\}_k \dashv \Xi
}{
	\Xi \vdash i \Translates_e i' \dashv \Xi &
	\Xi; \cdot_\Pattern \vdash p_k \Translates_p p_k' \dashv \Xi; \Pattern_k &
	\Pattern_k \mapsto \Scope_k &
	\Xi; \Scope_k \vdash e_k \Translates_e e_k' \dashv \Xi
}
\end{equation}
...
An example of this structure is illustrated in figure~\ref{figure:referencing-environment-implementation}.
...
Pretty-printing to \texttt{HTML} further assigns IDs to constants so that references to them can be printed as hyperlinks.
...
Traversing of the dictionaries and stacks in order of addition like is done in the formalism of section~\ref{section:formalising-unified-indexing} to count up to the correct de Bruijn index defeats the purpose of using dictionaries to speed up lookups.
Instead, as variables are added to the referencing environment, they are annotated with the previous size of their indexing context.
...
From a programming language implementation standpoint, the revised indexing phase simplified the flow of information by decoupling it from the signature reconstruction store.

...
A variable resolved at the \LF level is indexed with respect to only the \LF context, and likewise for variables at the meta and computation levels.
}
...
The abstract data type of referencing environments has the following operations, where $\Psi$ generically denotes a single indexing context:
\begin{itemize}
\item
...
Identifier lookup and index operations are separate since $\Lookup{x}{\Xi}$ considers the entire referencing environment whereas $\Index{\Psi}{x}{\Xi}$ is restricted to the indexing context $\Psi$ in which $x$ was previously resolved to belong by lookup in $\Xi$.

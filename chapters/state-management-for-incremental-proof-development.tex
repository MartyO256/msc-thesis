...
For instance, constraint-based type systems require mechanisms to keep track of the effects of solving contraints so that those effects can be reverted in the event that type-checking fails for a given program.
...
Furthermore, command histories may be implemented to support undoing and redoing commands, like in proof assistants since users may apply tactics that do not lead to a solution for a given subgoal.
Additionally, subgoal trees may also added to allow navigating between incomplete parts of a proof.
...
Since \Beluga is a functional programming language, every identifier declaration is allowed to shadow previous declarations.
The only admissible exception to this rule is that identifiers in mutually recursive declarations have to be unique.
